...
co::State co::modifiers::ls(co::DGraph &g, co::State &state, co::Sampler &sampler) {
    co::State best = state;
    // try this for every vertex
    for (int i = 0; i < g.V; ++i) {
        for (int j = 0; j < g.V; ++j) {
            if (i == j) {
                continue;
            ...
            co::State new_state = best;
            new_state.set_order(i, best.ordering[j]);
            new_state.set_order(j, best.ordering[i]);

            std::vector<int> changed = {i, j};
            new_state.evaluate_incremental(g, best, changed);
