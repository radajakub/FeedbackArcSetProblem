...
void co::repair::random_ordered(DGraph &g, State &s, co::op_change destroyed, std::mt19937 &rng) {
    // if too big fall back
    if (destroyed.size() > 6) {
        co::repair::random(g, s, destroyed, rng);
        return;
    }
    ...
    // build small graph for the deleted nodes and solve it exactly
    DGraph small_g = g.subgraph(destroyed);
    co::State best_small = co::branch_and_bound(small_g);

    for (int i = 0; i < best_small.V; ++i) {
        int vi = best_small.positions[i];
        int v = small_g.vertex_map[vi];
        s.place_vertex(v, positions[i], g);
    }
}

void co::repair::greedy_ordered(DGraph &g, State &s, co::op_change destroyed, std::mt19937 &rng) {
    // if too big fall back
    if (destroyed.size() > 6) {
        co::repair::greedy(g, s, destroyed, rng);
        return;
    }

    // build small graph for the deleted nodes and solve it exactly
    DGraph small_g = g.subgraph(destroyed);
    co::State small = co::branch_and_bound(small_g);

    int small_cost = small.cost;

    std::vector<int> vertices(destroyed.size());
    for (int i = 0; i < small.V; ++i) {
        vertices[small.positions[i]] = small_g.vertex_map[i];
    }

    // iterate over all positions
    int best_cost = std::numeric_limits<int>::max();
    int best_start_position = -1;

    for (int pos = 0; pos < s.V - small.V; ++pos) {
        co::State copy = s;
        for (int v = 0; v < small.V; ++v) {
            copy.place_vertex(vertices[v], pos + v, g);
        }
        if (copy.cost < best_cost) {
            best_cost = copy.cost;
            best_start_position = pos;
        }
    }

    for (int v = 0; v < small.V; ++v) {
        s.place_vertex(vertices[v], best_start_position + v, g);
    }
}
